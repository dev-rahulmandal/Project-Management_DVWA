-- VulnForge deterministic seed - SQLite (dev).
...
INSERT INTO organizations (id, name, slug, plan_tier) VALUES
  ...
  (2, 'Globex Inc', 'globex', 'starter');

-- Acme users: ids 1-2 | Globex: ids 3-4 | super-admin: id 5
INSERT INTO users (id, org_id, email, full_name, role, is_super_admin, password_hash, internal_notes) VALUES
  (1, 1, 'alice@acme.test',               'Alice Forrest', 'owner',  0, '$2b$12$bpwMRSLMArdyTjtVjP2BBOY7u58y7ydIxX5ZulhTNlzYi7dwzgut6', 'First enterprise customer. High value.'),
  (2, 1, 'charlie@acme.test',             'Charlie Dunn',  'member', 0, '$2b$12$pgYdCyAchuhh9M6RuWFNm.q6XEJZtF44OswHPW/6iHCystVQKmd.m', NULL),
  (3, 2, 'bob@globex.test',               'Bob Hale',      'owner',  0, '$2b$12$nMq92KWvZ1h8JvOtaubRvu2RGiIRnSOq12495beDVmsS/uTcUgRRG',  'Pilot account - do not churn.'),
  (4, 2, 'diana@globex.test',             'Diana Marsh',   'member', 0, '$2b$12$6VsOG.feapMJ4mnsJsMYJei9FPe/zcKXKnOhvx324s/FRaPd4WAlC',  NULL),
  (5, 1, 'superadmin@vulnforge.internal', 'Super Admin',   'owner',  1, '$2b$12$Yx34OfLjS5v0Cy4FfLWviOQfAGSkqGpDFWH6Q4/Ety.DXVwm2knWK',  'INTERNAL: super-admin service account');

-- Projects: Acme IDs 1-3, Globex 4-6 (sequential across orgs - BOLA surface)
...
INSERT INTO tasks (id, project_id, org_id, title, body, assignee_id, status, priority) VALUES
  (1, 1, 1, 'Design mockups',     'Figma link: internal.acme.test/designs', 2, 'done',        'high'),
  ...
  (5, 5, 2, 'Pen test scope doc', 'Draft by end of month.',                 3, 'open',        'medium'),
...
-- A registered OAuth client ("Connect with VulnForge" demo integration).
...
INSERT INTO invitations (org_id, email, role, token, expires_at) VALUES
  (1, 'newbie@acme.test', 'member', 'inv_seed_acme_pending_001', '2031-01-01T00:00:00');
