#!/usr/bin/env bash
# ============================================================================
#  VulnForge dev launcher (Linux / macOS) - the LAB face.
#    api : FastAPI  http://localhost:8081  (uvicorn --reload)
#    web : Next.js  http://localhost:8082  (next dev)
#  Counterpart to start-dev.bat. Runs BOTH servers in this terminal;
#  press Ctrl-C to stop both.   Usage:  ./start-dev.sh   (or: bash start-dev.sh)
#  Expose to another machine (e.g. a Windows host -> this VM):
#     VF_HOST=<vm-ip> ./start-dev.sh                          (IP:port, or VF_HOST=auto)
#     VF_HOST=vulnforge.test VF_API_HOST=api.vulnforge.test ./start-dev.sh
...
# both the host and the VM). print_access_panel() below lists every working URL.
BIND="127.0.0.1"
...
# --- free ports 8082/8081 (stop any stale dev servers) ----------------------
free_port() {
...
# --- ensure web deps (first run only) ---------------------------------------
[ -d web/node_modules ] || {
...
echo "[VulnForge] starting api (:8081) + web (:8082)..."
"$VPY" -m uvicorn api.main:app --host "$BIND" --port 8081 --reload &
API_PID=$!
( cd web && npm run dev -- -H "$BIND" ) &
WEB_PID=$!
