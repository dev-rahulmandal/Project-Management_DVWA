...
# Admin-only organization overview. The REAL authorization boundary is the
# require_admin dependency (owner/admin role, or super-admin) - the web app
# also hides this area, but client-side hiding is never the control. Counts are
# scoped to the caller's org. A broken-function-level-auth (BFLA) variant that
# drops/weakens this guard would be a separate, proposed, manifested endpoint.
