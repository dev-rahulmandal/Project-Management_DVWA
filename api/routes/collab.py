"""
Collaboration surface - notifications center + member directory.

Legitimate, correctly-secured product surface (Phase-B haystack):
  * notifications are scoped to the current user; you can only read/mark your own.
  * the member directory is org-scoped and returns SAFE fields only (no
    internal_notes / password) - deliberately NOT the excessive-data vuln.
"""
import aiosqlite
...
async def mark_read(
    ...
    # Scoped to the owner - you can only mark your OWN notification read.
...
async def member_directory(
    ...
    # Org-scoped directory; SAFE fields only (NOT internal_notes - that exposure
    # is the catalogued excessive-data vuln on GET /api/users/{id}, not here).
    async with db.execute(
