"""
Reporting & data-ops - benign org activity feed + data exports.

Legitimate, correctly-secured surface (Phase-B haystack):
  * GET /api/activity is the general org activity feed any member can see -
    deliberately distinct from the admin-only /api/admin/audit (the BFLA vuln):
    it is org-scoped and omits sensitive fields (no source IP).
  * exports are org-scoped to the caller's data only.
"""
import csv
import io
...
async def export_tasks(
    ...
    # Org-scoped export of the caller's tasks.
    async with db.execute(
