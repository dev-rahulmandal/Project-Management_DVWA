...
# Legitimate, correctly-scoped surface: you only see your own org's projects.
# This is the realistic list a crawler discovers; the BOLA is the detail route.
...
async def list_projects(
    ...
    # Soft-deleted projects are hidden by default; ?trashed=true lists the trash.
...
# Create a project in the CALLER'S OWN org. org_id/created_by_id are taken from
# the authenticated user, never the request body - a client cannot plant a
# project into another tenant (that would be an unmanifested write-side BOLA).
...
# Edit a project in the caller's org (org-scoped, field-validated).
...
# Soft-delete (move to trash) + restore. Org-scoped; reversible.
...
# Permanently delete a project and everything under it (tasks, comments, attachments).
...
# Benign product surface (NOT a vuln): tasks are scoped to the caller's org, so
# a cross-tenant project id returns an empty list rather than leaking tasks.
# A tasks-level BOLA, if we want one, is planted separately with its own
# manifest entry + tests - never smuggled in under a UI feature.
...
# Create a task under a project the caller's org owns. The project is looked up
# org-scoped first (404 otherwise), so a task can't be attached to another
# tenant's project - benign, correctly-scoped write.
