...
# Hidden from the OpenAPI schema: BFF-internal endpoint used by the web origin
# during login, not part of the public API surface.
...
async def verify_credentials(
    ...
    """
    Internal-only endpoint: used by the web origin to verify credentials
    during login. Returns the user record (without password_hash) on success.
...
# Hidden from the OpenAPI schema: BFF-internal endpoint used by the web origin
# during self-serve registration. Creates a brand-new org and its first user.
...
async def register_user(
    ...
    """
    Self-serve signup: creates a new organization and its owner.

    SECURITY-SENSITIVE FIELDS ARE NOT CLIENT-SETTABLE. role and is_super_admin
    are hardcoded here (owner of the new org / not a super-admin) - the request
    body only carries email/password/fullName/orgName. This is deliberately the
    correctly-scoped behavior; a mass-assignment hole here would be an
    unmanifested vuln, which the Two Laws forbid.
    """
    ...
    # Unique org slug (schema enforces UNIQUE) - append a counter on collision.
...
# Hidden from the OpenAPI schema: BFF-internal endpoint used by the web origin to
# accept an org invitation (creates the invited user, then consumes the invite).
...
async def accept_invite(
    ...
    try:
        ...
        pass  # tolerate non-ISO seed values
