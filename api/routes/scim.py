"""
SCIM 2.0 user provisioning (the secure baseline).

System-for-Cross-domain-Identity-Management endpoints an IdP (Okta/Entra/etc.)
calls to provision users into the org. Gated by the SCIM provisioning bearer
(config.SCIM_TOKEN) - a lower-trust automation credential, not an admin session.

Security model (the correct one):
  * The internal user id is ALWAYS server-generated. A client-supplied `externalId`
    is stored only as an opaque IdP label; it is NEVER used as, or matched against,
    an internal primary key.
  * Provisioned users are created as plain members in the integration's own org.
  * Just-in-time access is issued ONLY for the freshly-created user.
The scope-confusion / externalId-as-internal-id escalation lives in scim_vuln.py.
"""
import secrets
...
def require_scim_token(
    ...
    """Validate the IdP provisioning bearer; returns the bound org id."""
...
def _jwt_for(user_id: int) -> str:
    # Just-in-time provisioning grant for the provisioned identity.
...
async def scim_create(
    ...
    # AUTH-SCIM-001 - one route; hardened() picks the path.
    if not hardened(request):
        # VULN: externalId is trusted as the internal primary key. If a user with
        # that id already exists, "link" the SCIM identity to it and issue a JIT
        # grant for THAT internal user - externalId="5" yields super-admin access.
    ...
    # SECURE: internal id is auto-generated; externalId is stored as a label only,
    # never used as a primary key. New users are plain members of the bound org.
    full_name = (body.name.formatted if body.name else None) or body.userName
    pw = bcrypt.hashpw(secrets.token_urlsafe(24).encode(), bcrypt.gensalt(rounds=12)).decode()
    try:
        cur = await db.execute(
